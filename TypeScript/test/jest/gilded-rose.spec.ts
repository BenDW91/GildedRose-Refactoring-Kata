...
describe('Gilded Rose', () => {
  ...
  describe('Aged Brie', () => {
    it('increases quality by 1 before sell date', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.AGED_BRIE, 5, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(11);
    });

    it('increases quality by 2 after sell date has passed', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.AGED_BRIE, 0, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(12);
    });

    it('quality never exceeds 50', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.AGED_BRIE, 5, 50)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(50);
    });

    it('quality never exceeds 50 after sell date', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.AGED_BRIE, 0, 49)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(50);
    });
  });
  ...
  describe('Sulfuras', () => {
    it('never changes quality', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.SULFURAS, 5, 80)]);
      const items = gildedRose.updateQuality();
  ...
  describe('Backstage passes', () => {
    it('increases quality by 1 when more than 10 days remain', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 15, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(11);
    });

    it('increases quality by 2 when 10 days or fewer remain', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 10, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(12);
    });

    it('increases quality by 2 when exactly 6 days remain', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 6, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(12);
    });

    it('increases quality by 3 when 5 days or fewer remain', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 5, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(13);
    });

    it('increases quality by 3 when 1 day remains', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 1, 10)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(13);
    });

    it('drops quality to 0 after the concert', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 0, 30)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(0);
    });

    it('quality never exceeds 50', () => {
      const gildedRose = new GildedRose([new Item(ItemNames.BACKSTAGE_PASS, 5, 49)]);
      const items = gildedRose.updateQuality();
      expect(items[0].quality).toBe(50);
    });
  });
